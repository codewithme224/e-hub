<template>
      ...
      <div class="container p-6 mx-auto">
        <div class="flex items-center justify-between mb-8">
          ...
          <div class="flex items-center">
            <div class="flex items-center p-4 mr-4 bg-purple-900 bg-opacity-50 rounded-lg shadow-lg backdrop-filter backdrop-blur-lg animate-fadeIn">
              <img src="/assets/images/module/student.png" alt="Avatar" class="w-12 h-12 mr-4 border-2 border-pink-500 rounded-full" />
              <div>
                <p class="text-lg font-semibold">Welcome back, {{ user?.name }}!</p>
                <p class="text-sm text-gray-300">Always stay updated in your student portal</p>
              </div>
            </div>
            <button @click="logout" class="px-4 py-2 font-semibold text-white transition-all duration-300 rounded-lg shadow-lg bg-gradient-to-r from-red-500 to-pink-500 hover:from-pink-500 hover:to-red-500 hover:shadow-xl">
              Logout
            </button>
          </div>
        </div>
  ...
  const logout = async () => {
    try {
      ...
      toast.add({ severity: 'error', summary: 'Error', detail: error.response?.data?.message || 'Logout failed', life: 3000 });
    }
  };
